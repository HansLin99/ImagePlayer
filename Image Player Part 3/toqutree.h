...
private:
   ...
   */
   Node * buildTree(PNG * im, int k);
    ...
    PNG renderHelper(Node* subRoot);
    
    Node* copyHelper(const Node* curr);

    PNG* getPixel(int side, PNG* im, int offsetX, int offsetY);

    PNG getPixel4R(PNG* im, PNG children, int offsetX, int offsetY, int sideLength);

    int inTolerence(Node* subRoot, double tolerance, HSLAPixel avg);
